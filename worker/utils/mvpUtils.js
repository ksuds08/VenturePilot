...
/**
 * Generate a single component implementation by calling OpenAI.  This
 * helper retries generation once if an error occurs, returning null
 * on failure.  Components may represent either frontend or backend
 * pieces of the MVP.
 *
...
- If backend:
  - Create a file at: functions/api/${component.name}.ts
...
{
  "files": {
    "functions/api/${component.name}.ts": "..."
  }
}
...
Add this feature to the existing site.`.trim(),
    },
  ];
  try {
 ...
 * The returned object maps file paths to contents.  This helper
 * enforces Tailwind usage and ensures that index.html is present.
 *
 ...
 * generate each one by prompting OpenAI.  This helper first asks the
 * model for a list of files (with filename and purpose), then calls
 * OpenAI again to generate the code for each file.  Files are
 * returned in a single object mapping filenames to contents.
 *
...
- Return only valid JSON: { "files": { "${file.filename}": "..." } }
`.trim(),
      },
      {
        role: 'user',
...
`.trim(),
    ...
    try {
      ...
      console.error(`❌ Failed to generate subfile: ${file.filename}`, fileRaw);
    }
  }
  return outputFiles;
}
 ...
 * components.  This helper prompts the model to perform the
 * decomposition and filters out non‑functional items such as logos or
 * themes.
 *
...
/**
 * Generate a router index file for Pages Functions based on the
 * discovered backend handlers.  Each handler must export an async
 * function matching the naming convention `${name}Handler`.
 *
