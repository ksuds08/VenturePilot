 ...
 * 'worktop'.
 *
 * @param {string} code â TypeScript or JavaScript source code
 * @returns {string} â sanitized code with disallowed imports removed
 */
 ...
 *
 * @param {Object} component â component definition (backend type)
 * @param {Object} plan â full MVP plan
 * @param {Object} env â environment variables
 * @returns {Promise<Object|null>} â map of filenames to code or null on failure
 */
export async function generateBackendComponentFiles(component, plan, env) {
  const filePath = `functions/api/${component.name}.ts`;
  const sysLines = [
    'You are a senior fullâstack engineer. Generate all necessary backend files for the given component in a single response.',
    '',
    'Requirements:',
    '- Use only native Cloudflare Worker APIs (Request, Response, fetch).',
    '- DO NOT import any modules or packages other than builtâin APIs. Do not import from "cloudflare-worker-types", "some-cloudflare-package", "undici", "worktop", or any other external library.',
  ...
  try {
    ...
    console.error('â Failed to generate backend files for component', component.name, err.message);
    return null;
  }
}
...
export async function generateComponentSubFiles(component, plan, env) {
  ...
  try {
    ...
    try {
      ...
      console.error(`â Failed to parse subâfile list for component: ${component.name}`, err2.message);
      return {};
    }
  }
  const outputFiles = {};
...
export async function decomposePlanToComponents(plan, env) {
  ...
  try {
    ...
    if (msg?.function_call && msg.function_call.name === 'list_components') {
      // Parse the arguments string returned from the function call. Since the
      // schema defines a topâlevel object with a `components` array, extract
      // that property if it exists; otherwise assume the parsed value is the
      // array itself (for backward compatibility).
      const args = msg.function_call.arguments || '{}';
      const parsedArgs = JSON.parse(args);
      components = parsedArgs?.components ?? parsedArgs;
    } else {
      // When no function call occurs, fallback to the JSON chat helper. It may
      // return either a plain array (legacy) or an object with a `components`
      // property. Normalize to an array accordingly.
