...
export default function useChatStages(onReady?: () => void) {
  ...
  const handleConfirmBuild = async (id: any) => {
    ...
    try {
      // Make the API call to build and deploy. Sanitize messages before sending.
      ...
      // Attempt to parse JSON; if parsing fails, treat the body as a plan string.
      let data: any;
      try {
        data = await res.json();
      } catch {
      ...
      // Success: if we have a deployment URL, show it; otherwise display the plan.
      if (data.pagesUrl || data.workerUrl) {
        ...
        // No deployment URL: show the plan instead of a live site
        messageAccumulator = [
          ...messageAccumulator,
          {
            role: "assistant",
            content:
              "✅ Here’s the MVP build plan generated by the agent:\n\n" +
              data.plan,
          },
        ];
        updateIdea(id, {
          deploying: false,
          // We did not deploy, so don't set pagesUrl
          messages: [...messageAccumulator],
          takeaways: {
            ...idea.takeaways,
            finalPlan: data.plan,
        ...
        messageAccumulator = [
          ...
          {
            role: "assistant",
            content: "⚠️ Unexpected response from build agent.",
          },
        ];
        updateIdea(id, {
          deploying: false,
